...
- **Remote protocol mirror rule**: `src/shared/remote-protocol.ts` is the source of truth for the wire contract with the iPad client; `Sources/ClaveProtocol/RemoteProtocol.swift` in the separate `clave-ios` repo mirrors it. Any wire change (message types, session fields, capabilities, chat event schema) updates both in the same change, and new server message shapes get a captured fixture line in the iOS repo's `Tests/ClaveKitTests/Fixtures/wire-messages.jsonl`.
